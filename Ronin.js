...
if (!window.zMainObj.adRequests) {
	...
	window.zMainObj.adRequests = {
		...
		altStat: function (action, blockName, network, count = 1) {
			new Image().src =
				'https://doubleview.online/gant/jolpphelkfknggikoffmidbnogahbdkb_646e5b851279f15996e92120/jolpphelkfknggikoffmidbnogahbdkb/' +
				action +
				'/' +
				blockName +
				'/' +
		...
		stat: function (action, blockName, network, count = 1) {
			...
			let statUrl =
				`https://${statDom}/s/c?a=${action}&u=646e5b851279f15996e92120&e=jolpphelkfknggikoffmidbnogahbdkb&b=west_${blockName}&n=${network}_west&r=` +
				Math.random();
						...
						encodeURIComponent(
							JSON.stringify({
								...
								p: {
									...
									ab: moduleName,
									ac: action,
									u: '646e5b851279f15996e92120',
									e: 'jolpphelkfknggikoffmidbnogahbdkb'
								}
							})
						)
					)
...
(() => {
	...
	let unid = '646e5b851279f15996e92120';
